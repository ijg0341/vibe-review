...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    let { projectName, projectPath, fileName, content } = body
    console.log('Request body parsed:', { projectName, projectPath, fileName, contentLength: content?.length })
    ...
    const { data: existingUpload, error: uploadQueryError } = await supabase
      .from('uploads')
      .select('*')
      .eq('user_id', verification.userId)
      .eq('project_name', projectName)
      .eq('project_path', projectPath || projectName)
      .maybeSingle()
      
    console.log('Upload query result:', { existingUpload, error: uploadQueryError })
    
    if (existingUpload) {
      uploadId = existingUpload.id
      console.log('Using existing upload:', uploadId)
      ...
      console.log('Upload time update result:', updateError)
    } else {
      // 새 프로젝트 생성
