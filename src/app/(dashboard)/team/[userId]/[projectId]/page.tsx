...
export default function TeamProjectDetailPage() {
  ...
  const [projectOwner, setProjectOwner] = useState<TeamMember | null>(null)
  ...
  const fetchSessionLines = async (fileId: string, page: number = 1, search?: string, append: boolean = false) => {
    try {
      ...
      if (page === 1) {
        ...
        if (infoError) {
          // RPC 함수가 없을 경우 직접 쿼리 사용 (에러 로그 없이)
      ...
      // 실제 라인 데이터 가져오기 - RPC 함수 대신 직접 쿼리 사용
      let data;
      let error;
      ...
      try {
        ...
        if (rpcResult.error) {
          // RPC 함수가 없거나 권한 문제 - 조용히 fallback 사용
                      ...
                      <div className="flex items-center gap-2 px-2 py-1 bg-muted rounded-md">
                        <Avatar className="h-6 w-6">
                          ...
                          <AvatarFallback className="text-xs">
                            {getUserInitials(projectOwner.email, projectOwner.display_name)}
                          </AvatarFallback>
                        </Avatar>
                        <span className="text-sm font-medium">
                          {projectOwner.display_name || projectOwner.email}
                        </span>
                      </div>
                    )}
